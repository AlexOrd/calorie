...
<div class="flex flex-col gap-5">
  ...
  <p class="border-border bg-surface rounded-md border p-3 text-sm">
    Приклад: {sample}
  </p>
  <div class="flex justify-between gap-3">
    <button
      type="button"
      class="border-border bg-surface min-h-14 rounded-lg border px-6 py-4 text-base"
      onclick={onBack}
    >
      Назад
    </button>
    <button
      type="button"
      class="bg-accent text-on-accent min-h-14 flex-1 rounded-lg px-6 py-4 text-lg font-semibold shadow-md shadow-black/20"
      onclick={onConfirm}
    >
      Готово
    </button>
